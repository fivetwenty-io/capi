...
func DefaultNATSKVConfig() *NATSKVConfig {
	return &NATSKVConfig{
		...
		MaxValueSize: constants.MaxCacheValueSize, // 1MB
		History:      1,
		Replicas:     1,
	}
}
...
func NewNATSKVCache(config *NATSKVConfig) (*NATSKVCache, error) {
	...
	// Connect to NATS
	natsConn, err := nats.Connect(config.URL, opts...)
	if err != nil {
	...
	keyValue, err := jetStream.CreateOrUpdateKeyValue(context.Background(), kvConfig)
	if err != nil {
		natsConn.Close()
...
func (c *NATSKVCache) Get(ctx context.Context, key string) (*CacheEntry, error) {
	...
	err = json.Unmarshal(entry.Value(), &wrapper)
	if err != nil {
...
func NATSKVCacheWithConnection(natsConn *nats.Conn, bucket string, ttl time.Duration) (*NATSKVCache, error) {
	// Create JetStream context
	jetStream, err := jetstream.New(natsConn)
	if err != nil {
		return nil, fmt.Errorf("failed to create JetStream context: %w", err)
